...

{
    ...
    void SimulateEngine()
    {
        ...
        // --- THE PROGRESSIVE BRAKE FIX ---
        float baseDrag = 0.05f;
        if (!engineRunning) baseDrag = (currentGear == 0) ? 0.005f : 5.0f;
        else if (clutch > 0.8f) baseDrag = 0.005f;
        else baseDrag = (throttle < 0.1f) ? 0.15f : 0.05f;

        if (smoothedBrakeInput > 0.01f)
        {
            rb.drag = Mathf.Lerp(baseDrag, 2.0f, smoothedBrakeInput);
        }
        else
        {
            rb.drag = baseDrag;
